...
function drawLines(filteredData, x, y, xDomain, svg, states) {
  ...
  states.forEach(state => {
    ...
    const line = svg.append("path")
      ...
      .attr("class", "line")
      .attr("fill", "none")
      .attr("stroke", color.primary) // Apply the subtle red color to all lines
      .attr("stroke-width", 1.5)
      .attr("d", d3.line()
        .x(d => x(d.month))
        .y(d => y(d.count))
    ...
    line.on("mouseover", function(event, d) {
      d3.select(this).attr("stroke", color.highlight).attr("stroke-width", 3); // Highlight with a different color
    ...
    .on("mouseout", function() {
      d3.select(this).attr("stroke", color.primary).attr("stroke-width", 1.5); // Revert to original color on mouse out
...
export function createLineChart(data) {
  ...
  drawLines(nestedData, x, y, months, svg, states);
  addEventListeners();
}